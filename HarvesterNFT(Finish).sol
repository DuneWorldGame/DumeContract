    ...
    OwnableUpgradeable
{
    ...
    struct PassengersInterval {
        uint256 base;
        uint256 stepSize;
    }

    mapping(uint256 => NftInfo) internal _nftInfo;
    mapping(uint256 => PassengersInterval) internal _passengersInterval;

    function nftInfo(uint256 _id) external view returns (NftInfo memory) {
        require(
            _nftInfo[_id].updateBlockNumber != block.number,
            "Please go to the next block to check"
        );
        return _nftInfo[_id];
    }

    function passengersInterval(uint256 _type)
        public
        view
        returns (PassengersInterval memory)
    {
        return _passengersInterval[_type];
    }
    ...
    function setPassengers(
        ...
        _passengersInterval[i] = PassengersInterval({
            base: base,
            stepSize: stepSize
        });
        return true;
    ...
    function setMysteryBoxs(uint256 _index, address _addr)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function setPossibilitys(uint256 _index, uint16[5] memory _possibilitys)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function setDistributor(address _address, bool _state)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function randomToLevel(uint256 _type) internal view returns (uint256) {
        uint256 _random = IRandom(randomAddress).random(
            msg.sender,
            block.number,
            10000
        );
    ...
    function randomToPassengers(uint256 _level)
        internal
        view
        returns (uint256)
    {
        uint256 base = _passengersInterval[_level].base;
        uint256 stepSize = _passengersInterval[_level].stepSize;
        uint256 _random = IRandom(randomAddress).random(
            msg.sender,
            block.number,
            stepSize
    ...
    function mint_(address _to, uint256 _type) internal returns (bool) {
        ...
        uint256 _randomToPassengers = randomToPassengers(_randomToLevel);

        _nftInfo[thisId] = NftInfo({
            updateBlockNumber: block.number,
            level: _randomToLevel,
            passengers: _randomToPassengers,
