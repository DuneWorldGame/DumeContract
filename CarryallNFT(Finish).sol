...
contract CarryallNFT is Initializable,ERC721HolderUpgradeable,ERC721EnumerableUpgradeable ,OwnableUpgradeable{
    ...
    struct EscapeInterval {
        uint256 base;
        uint256 stepSize;
    }

    mapping(uint256 => NftInfo) internal _nftInfo;
    mapping(uint256 => EscapeInterval) internal _escapeInterval;
    ...
    function escapeInterval(uint256 _type) public view returns(EscapeInterval memory) {
        return _escapeInterval[_type];
    }
    ...
    function setEscapeInterval(
        uint256 i,
        uint256 base,
        uint256 stepSize
        ) public onlyOwner returns(bool) {
        _escapeInterval[i] = EscapeInterval({
            base:base,
            stepSize:stepSize
        });
        return true;
    ...
    function initialize(string memory _name,string memory _symbol,address _randomAddress) external initializer {
       ...
       setEscapeInterval(0,5000,1000);
       setEscapeInterval(1,6000,1000);
       setEscapeInterval(2,7000,1000);
       setEscapeInterval(3,8000,1000);
       setEscapeInterval(4,9000,1000);
    ...
    function randomToEscape(uint256 _level) internal view returns(uint256) {
        uint256 base = _escapeInterval[_level].base;
        uint256 stepSize = _escapeInterval[_level].stepSize;
    ...
    function mint_(address _to,uint256 _type) internal returns(bool){
        ...
        uint256 _randomToEscape = randomToEscape(_randomToLevel); 

        _nftInfo[thisId] = NftInfo({
            updateBlockNumber:block.number,
            level:_randomToLevel,
            escape:_randomToEscape,
